...
func buildServer(env config.EnvVars) (*fiber.App, func(), error) {
	...
	webhookStore := webhook.NewWebhookStorage(db)
	webhookController := webhook.NewWebhookController(webhookStore)
	webhook.AddWebhookRoutes(app, env.WEBHOOK_ROUTE, webhookController)
